    ...
    const fetchEleves = async () => {
      try {
        ...
        const response = await axios.get(
          'http://192.168.65.1:8000/api/chauffeur/eleves',
          {
            headers: {
        ...
        console.error('Erreur fetch élèves:', error.response?.data || error.message);

        if (error.response?.status === 401) {
          Alert.alert('Session expirée', 'Veuillez vous reconnecter.');
          router.replace('/');// <-- remplacer par la page existante si nécessaire
        } else {
