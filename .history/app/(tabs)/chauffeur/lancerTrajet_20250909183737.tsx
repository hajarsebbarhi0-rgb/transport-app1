...
export default function LancerTrajet() {
  ...
  // 🔹 Envoi de la position vers Laravel (avec token)
  const sendLocationToServer = async (location: Location.LocationObject) => {
    if (!trajetId) return;
    try {
      const token = await AsyncStorage.getItem('userToken');
      await axios.post(
        `${API_URL}/send-location`,
        {
          latitude: location.coords.latitude,
          longitude: location.coords.longitude,
        },
        {
          headers: { Authorization: `Bearer ${token}` }, // ✅ Auth Sanctum
        }
      );
      console.log("📍 Position envoyée :", location.coords);
    } catch (error) {
