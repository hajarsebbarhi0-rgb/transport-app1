...
export default function ListeEleves() {
  ...
  useEffect(() => {
    const fetchEleves = async () => {
      try {
        // 🔐 Récupération du token correct
        const token = await AsyncStorage.getItem('userToken'); // <-- ici
        if (!token) {
        ...
        // Gestion spéciale pour 401
        if (error.response?.status === 401) {
          Alert.alert('Erreur', 'Session expirée ou non autorisée. Veuillez vous reconnecter.');
          router.replace('/login'); // Redirection vers l'écran de login
        } else {
          Alert.alert(
            'Erreur',
