  ...
  const fetchProfile = async () => {
    try {
      ...
      const response = await axios.get<Eleve>(
        "http://192.168.2.152:8000/api/eleve/profile",
        {
    ...
    if (!pickerResult.canceled) {
      ...
      try {
        const response = await axios.post(
          "http://192.168.2.152:8000/api/eleve/photo",
          formData,
          {
            headers: {
  ...
  return (
    <ScrollView contentContainerStyle={styles.container}>
      <View style={styles.photoContainer}>
        <Image
          source={{ uri: eleve.photo ? `http://192.168.2.152:8000/${eleve.photo}` : undefined }}
          style={styles.photo}
        />
...
const styles = StyleSheet.create({
  ...
  input: { backgroundColor: "#fff", padding: 10, borderRadius: 10, marginBottom: 15, fontSize: 16, color: "#474b4cff" },
