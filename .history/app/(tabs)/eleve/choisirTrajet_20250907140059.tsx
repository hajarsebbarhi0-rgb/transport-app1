...
export default function ChoisirTrajet() {
  ...
  // Charger les arrêts quand un trajet est choisi
  const handleTrajetSelect = async (trajetId: number) => {
    setSelectedTrajet(trajetId);
    try {
      const res = await axios.get(`http://192.168.189.1:8000/api/trajets/${trajetId}/arrets`);
      setArrets(res.data);
    } catch (error) {
      console.error(error);
  ...
  const handleSaveChoice = async () => {
    ...
    try {
      ...
      const userId = await AsyncStorage.getItem("userId"); // ⚠️ Sauvegarde userId au login si pas déjà fait
      ...
      router.replace("/(tabs)/eleve"); // Retour tableau de bord élève
    } } catch (err) {
  const error = err as any; // on caste en any pour accéder aux champs
