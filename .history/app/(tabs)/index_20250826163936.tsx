...
const LoginScreen = () => {
  ...
  const handleLogin = async () => {
    try {
      // ⚠️ IMPORTANT : L'adresse IP doit être l'adresse IPv4 de votre PC
      //              Vérifiez-la en utilisant "ipconfig" ou "ifconfig"
      const response = await axios.post('http://192.168.226.1:8000/api/login', {
        email: email,
        password: password,
        role: role,
      });
      ...
      if (user.role === 'chauffeur') {
        // Correction ici : redirigez simplement vers le dossier "chauffeur"
      ...
      // ✅ Correction ici : le cast de l'erreur en type 'any' élimine l'avertissement de type 'unknown'
...
const styles = StyleSheet.create({
  container: {
    ...
    alignItems: 'center',
    backgroundColor: '#f0f4f7', // Fond plus doux
  },
  card: {
  ...
  roleContainer: {
    ...
    width: '100%',
    marginBottom: 30,
    gap: 10, // Ajout d'un espacement entre les boutons
  },
  roleButton: {
    flex: 1,
    padding: 15,
