...
const RegisterScreen = () => {
  ...
  const [codeTrajet, setCodeTrajet] = useState('');
  const [isLoading, setIsLoading] = useState(false);

  const handleRegister = async () => {
    if (!telephone || !email || !password || !confirmPassword || !nom || !prenom || !codeTrajet) {
    ...
    try {
      const response = await axios.post('http://192.168.2.16:8000/api/register', {
        email,
        password,
        nom,
        prenom,
...
const styles = StyleSheet.create({
  container: { flex: 1, backgroundColor: '#e8e6df' },
