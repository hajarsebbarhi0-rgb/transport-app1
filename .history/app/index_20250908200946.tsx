...
const LoginScreen = () => {
  ...
  const handleLogin = async () => {
    ...
    setLoading(true);
    try {
      const response = await axios.post('http://192.168.205.1:8000/api/login', {
        email,
        password,
        role,
      });
      ...
      Alert.alert(
        'Erreur',
        error.response?.data?.message || 'Identifiants ou rôle invalides.'
      );
    } finally {
      setLoading(false);
    }
...
const styles = StyleSheet.create({
  ...
  roleButton: { flex: 1, padding: 15, borderWidth: 1, borderColor: '#c8d0d2', borderRadius: 10, alignItems: 'center' },
  selectedRole: { backgroundColor: '#c8d0d2' },
  loginButton: { backgroundColor: '#b13623', padding: 18, borderRadius: 10, width: '100%', alignItems: 'center' },
  buttonText: { color: '#fff', fontWeight: 'bold', fontSize: 16 },
  registerLinkText: { color: '#4a8e9e', fontSize: 14, fontWeight: 'bold' },
});
